function quadrotor_sim
	...
	uav_dynamics.x = [0; 0; -4];    %initial position of uav [m]
	...
	init_attitude(1) = deg2rad(190); %roll
...
function motor_thrust = quadrotor_thrust_allocation(quad, M, f)
...
function quadrotor_thrust_to_dynamics(motors_thrust, R)
	d = 0.45; %[m], distance from center of gravity to motor

	cg_to_m1 = [cos(deg2rad(45)), sin(deg2rad(45)), 0];   %motor1 position
	cg_to_m2 = [cos(deg2rad(135)), sin(deg2rad(135)), 0]; %motor2 position
	cg_to_m3 = [cos(deg2rad(225)), sin(deg2rad(225)), 0]; %motor3 position
	cg_to_m4 = [cos(deg2rad(315)), sin(deg2rad(315)), 0]; %motor4 position

	M_m1 = cross(cg_to_m1, motors_thrust(1)); %moment generated by motor1
	M_m2 = cross(cg_to_m2, motors_thrust(2)); %moment generated by motor2
	M_m3 = cross(cg_to_m3, motors_thrust(3)); %moment generated by motor3
	M_m4 = cross(cg_to_m4, motors_thrust(4)); %moment generated by motor4

	%composite moment of all motors
	M = [0, 0, 0];
	M = M_m1 + M_m2 + M_m3 + M_m4;
	%composite force of all motors
	F = motors_thrust(1) + motors_thrust(2) + motors_thrust(3) + motors_thrust(4);
end
